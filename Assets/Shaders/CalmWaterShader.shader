Shader "Unlit/CalmWaterShader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float computeAlpha(float value);
            ...
            float4 frag (FragmentAttributes input) : SV_Target
            {
                ...
                float3 tex1Color = tex2D(_CalmWaveTex, scrolledStTex1St * 0.8);
                float3 tex2Color = tex2D(_CalmWaveTex2, scrolledStTex2St * 0.8);

                float3 outColor = tex1Color + tex2Color;
                float alpha = computeAlpha(outColor.r);
