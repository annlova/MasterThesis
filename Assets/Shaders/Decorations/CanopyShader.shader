    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            FragmentAttributes vert (appdata_full v)
            {
                ...
                float4 localPos = v.vertex + float4(offset, 0.0f);
            ...
            float4 frag (FragmentAttributes i) : SV_Target
            {
                ...
                float3 diffuseColor = tex2D(_DistributionTexDiffuse, i.rng);
                // float3 diffuseColor = _Diffuse * lerp(0.5f, 1.5f, i.rng);
                
                // Specular term
                float specularStrength = _Smoothness * step(1.0f, attenuation);
