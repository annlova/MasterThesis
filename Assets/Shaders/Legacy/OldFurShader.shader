...
//        Tags {"RenderType"="Opaque"}
        ...
        Pass
        {
            ...
            float4 frag (vertexOutput IN) : COLOR
            {
                ...
                float windFactor = ((snoise(IN.WORLD_POS.xz * 0.2f+ float2(_Time.y, _Time.y)) + 1.0f) / 2.0f) * 0.1f;
                float pedroDistance = distance(_PedroWorldPos, IN.WORLD_POS.xyz);
                float3 pedroDir = normalize(_PedroWorldPos - IN.WORLD_POS.xyz);
                // st += -pedroDir.xz * k * (1.0f - smoothstep(0.0f, 1.0f, pedroDistance));
                // st -= windFactor * k * _Falloff;
                ...
                float variation = 0.1f * _ColorVariation * k;
                float4 color = tex2D(_FurTexture, oldST);//* lerp(1.0 - variation, 1.0f, hair);// * clamp(random(ipos + 5.0f), _ColorVariation, 1.0f);
                color.r += lerp(0, variation, windFactor);
                color.g += lerp(0, variation, windFactor);
                color.b += lerp(0, variation, windFactor);
